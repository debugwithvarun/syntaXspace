...
export function ProfileRecieve() {
    ...
    return (
        <div className="flex w-full min-h-20 flex-col gap-6">
            <ItemGroup>
                {loading ? <ProfileShimmer/> :( people.length>0?  (people.map((person, index) => (
                    <React.Fragment key={person.username}>
                        ...
                        <AvatarImage src={`${ImagePath(person.profilepic)}`} alt="profilepic"></AvatarImage>
                            ...
                            <ItemGroup className="flex gap-2 flex-row">
                                <Button variant="outline" className="rounded-xl hover:bg-destructive text-destructive" onClick={()=>RemoveInvite(person.username)}>
                                    Ignore
                                </Button>
                                <Button variant="outline" className="rounded-xl hover:bg-green-600  text-green-600" onClick={()=>AcceptInvite(person.username)}>
                                    Accept
                                </Button>
                            </ItemGroup>
                        </Item>
                ...
                ))):<h6 className="min-h-18 w-full flex items-center justify-center text-sm ">No Pending Invitations</h6>)
